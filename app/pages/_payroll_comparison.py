...
def _load_employees(_cid: str = "") -> dict:
    ...
    result = (
        db.table("employees")
        .select("id, employee_no, first_name, last_name, basic_salary, position")
        .eq("company_id", get_company_id())
        .order("last_name")
        .execute()
    )
...
def _matches_pc_filter(emp_name: str, pos: str = "") -> bool:
    """Return True if the employee passes the payroll comparison filters."""
    f_name = (st.session_state.get("pc_f_name") or "").lower().strip()
    f_pos  = st.session_state.get("pc_f_pos")  or ""
    if f_name and f_name not in emp_name.lower():
        return False
    if f_pos and f_pos != pos:
        return False
    return True
...
def render():
    ...
    for emp_id in all_emp_ids:
        ...
        if emp is None:
            ...
            pos_str = emp.get("position") or ""

        if not _matches_pc_filter(name, pos_str):
            continue
