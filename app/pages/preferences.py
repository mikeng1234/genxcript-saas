...
PAGES_LIST = [
    "Dashboard", "Employees", "Payroll Run", "Payslips",
            ...
            for col, (key, theme) in zip(cols, row):
                with col:
                    ...
                    card_html = (
                        ...
                        f'<div style="font-size:20px;margin-bottom:4px;">{theme["emoji"]}</div>'
...
def _render_notifications():
    ...
    coming_soon = [
        ("📧 Payroll deadline reminders", "Get an email X days before each pay date"),
        ("🏛️ Government remittance alerts", "SSS, PhilHealth, Pag-IBIG, BIR due-date notices"),
        ("📋 Leave request notifications", "Email when an employee submits a leave request"),
        ("💰 Payslip availability", "Notify employees when their payslip is ready"),
        ("⚠️ Compliance warnings", "Alert when statutory contributions are out of date"),
    ]
