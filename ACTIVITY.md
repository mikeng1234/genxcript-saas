...
2026-03-16 [DesktopPC] Employee Master File improvements: Position + Department dropdowns now show existing values with "Add new" option (immediate text input, no submit needed); both fields auto-uppercase while typing; Department saved to employee_profiles with company_id (RLS fix); Department shown as caption under Position in employee list
2026-03-16 [DesktopPC] Unsaved changes guard: navigating away from an open employee edit form shows a dialog ("Stay & Keep Editing" / "Discard & Leave"); implemented via current_page vs nav_page split in main.py, reverting radio key before widget instantiation to avoid StreamlitAPIException
2026-03-16 [DesktopPC] Activity Log enhancements: employee updates now log before/after diffs (e.g. "Position: CASHIER → ACCOUNTANT", "Salary: ₱18,000 → ₱22,000"); old value shown in red, new value in green; added search box filtering across name, action, field names, and values
2026-03-16 [DesktopPC] Login fix: Employee ID lookup now uses ilike() for case-insensitive match (fixes "a00004" vs "A00004" mismatch); improved error messages distinguish "ID not found" from "no portal access set up"
